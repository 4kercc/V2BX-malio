...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/4kercc/V2BX-malio/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/4kercc/V2BX-malio/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
config() {
    echo -e "
  ${green}V2bX 配置修改方式：${plain}
————————————————
  ${green}1.${plain} 交互式快捷修改 (ApiHost, ApiKey, NodeID, 证书域名)
  ${green}2.${plain} 使用 vi 编辑器手动编辑完整配置文件 (/etc/V2bX/config.json)
  ${green}0.${plain} 返回主菜单
"
    read -rp "请输入选择 [0-2]: " cfg_choice
    case "$cfg_choice" in
        1)
            quick_modify_config
            ;;
...
# 快捷交互式配置修改
quick_modify_config() {
    local conf_file="/etc/V2bX/config.json"
    if [[ ! -f "$conf_file" ]]; then
        echo -e "${red}未找到配置文件: $conf_file${plain}"
        return 1
    fi

    # 读取当前配置
    local cur_host cur_key cur_id cur_domain
    cur_host=$(grep -oP '"ApiHost"\s*:\s*"\K[^"]+' "$conf_file" | head -n 1)
    cur_key=$(grep -oP '"ApiKey"\s*:\s*"\K[^"]+' "$conf_file" | head -n 1)
    cur_id=$(grep -oP '"NodeID"\s*:\s*\K[0-9]+' "$conf_file" | head -n 1)
    cur_domain=$(grep -oP '"CertDomain"\s*:\s*"\K[^"]+' "$conf_file" | head -n 1)

    echo ""
    echo -e "${yellow}当前配置项：${plain}"
    echo -e "  ApiHost    : ${green}${cur_host:-未配置}${plain}"
    echo -e "  ApiKey     : ${green}${cur_key:-未配置}${plain}"
    echo -e "  NodeID     : ${green}${cur_id:-未配置}${plain}"
    echo -e "  CertDomain : ${green}${cur_domain:-未配置}${plain}"
    echo ""

    read -rp "请输入新的 ApiHost (回车保持不变 [${cur_host}]): " input_host
    read -rp "请输入新的 ApiKey (回车保持不变 [${cur_key}]): " input_key
    read -rp "请输入新的 NodeID (回车保持不变 [${cur_id}]): " input_id
    read -rp "请输入新的 证书域名/SNI (回车保持不变 [${cur_domain}]): " input_domain

    local changed=false

    if [[ -n "$input_host" && "$input_host" != "$cur_host" ]]; then
        sed -i -E "s/(\"ApiHost\"\s*:\s*\")[^\"]+(\")/\1${input_host//\//\\/}\2/g" "$conf_file"
        changed=true
    fi
    if [[ -n "$input_key" && "$input_key" != "$cur_key" ]]; then
        sed -i -E "s/(\"ApiKey\"\s*:\s*\")[^\"]+(\")/\1${input_key}\2/g" "$conf_file"
        changed=true
    fi
    if [[ -n "$input_id" && "$input_id" != "$cur_id" ]]; then
        sed -i -E "s/(\"NodeID\"\s*:\s*)[0-9]+/\1${input_id}/g" "$conf_file"
        changed=true
    fi
    if [[ -n "$input_domain" && "$input_domain" != "$cur_domain" ]]; then
        sed -i -E "s/(\"CertDomain\"\s*:\s*\")[^\"]+(\")/\1${input_domain}\2/g" "$conf_file"
        changed=true
    fi

    if [[ "$changed" == "true" ]]; then
        echo -e "${green}配置修改成功！正在重启 V2bX 服务...${plain}"
        restart 0
    else
...
set_config_param() {
    ...
    if [[ ! -f "$conf_file" ]]; then
        echo -e "${red}错误：未找到配置文件 $conf_file${plain}"
        exit 1
    fi

    if [[ -z "$val" ]]; then
        echo -e "${red}错误：参数值不能为空！${plain}"
        echo -e "用法示例："
        echo -e "  v2bx set host https://dy.jsq.mk"
        echo -e "  v2bx set key jsqs"
        echo -e "  v2bx set id 30"
        echo -e "  v2bx set domain jp.gpfggtech.xyz"
        exit 1
    fi

    case "$key" in
        "host"|"apihost"|"ApiHost")
            sed -i -E "s/(\"ApiHost\"\s*:\s*\")[^\"]+(\")/\1${val//\//\\/}\2/g" "$conf_file"
            echo -e "${green}✓ 已将 ApiHost 更新为: ${val}${plain}"
            ;;
        "key"|"apikey"|"ApiKey")
            sed -i -E "s/(\"ApiKey\"\s*:\s*\")[^\"]+(\")/\1${val}\2/g" "$conf_file"
            echo -e "${green}✓ 已将 ApiKey 更新为: ${val}${plain}"
            ;;
        "id"|"nodeid"|"NodeID")
            if ! [[ "$val" =~ ^[0-9]+$ ]]; then
                echo -e "${red}错误：NodeID 必须为纯数字！${plain}"
                exit 1
            fi
            sed -i -E "s/(\"NodeID\"\s*:\s*)[0-9]+/\1${val}/g" "$conf_file"
            echo -e "${green}✓ 已将 NodeID 更新为: ${val}${plain}"
            ;;
        "domain"|"certdomain"|"CertDomain")
            sed -i -E "s/(\"CertDomain\"\s*:\s*\")[^\"]+(\")/\1${val}\2/g" "$conf_file"
            echo -e "${green}✓ 已将 CertDomain 更新为: ${val}${plain}"
            ;;
        *)
            echo -e "${red}错误：不支持的参数名 '$key'${plain}"
            echo -e "可选参数: host | key | id | domain"
            exit 1
            ;;
    esac
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/4kercc/V2BX-malio/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
        ...
        if [ ! -L /usr/bin/v2bx ]; then
            ln -s /usr/bin/V2bX /usr/bin/v2bx 2>/dev/null || true
        fi
...
show_menu() {
    echo -e "
  ${green}V2bX-malio 后端管理脚本，${plain}${red}不适用于docker${plain}
--- https://github.com/4kercc/V2BX-malio ---
  ${green}0.${plain} 修改配置
————————————————
